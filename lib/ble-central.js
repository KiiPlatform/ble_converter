...
BLECentral.prototype.start = function(){
  ...
  // fuction to handle discovered peripheral
  var onDiscover = function(peripheral) {
    var localName = peripheral.advertisement.localName;
    var deviceId
    // for the unconnectable devices, can get states in disconver phase.
    if (localName === 'BLECAST_BL' || localName === 'BLECAST_BL\u0000') {
      temp = new BlecastBL(peripheral);
      deviceId = temp.vendorThingID;
    ...
    }else if(localName === 'Nexturn') { // for the connectable devices, need to get state after device connected.
      var nexturnLed = new NexturnLED(peripheral);
      deviceId = nexturnLed.vendorThingID;

      // if the device is connecting, do nothing
      if (peripheralMap[deviceId] !== undefined && peripheralMap[deviceId] !== null){
        return;
      }

      // record this device in the map
  ...
  // check updates of blecast_bl, if the last update is earlier than 10 secondes, then report disconnection
  // for this unconnectable devices
